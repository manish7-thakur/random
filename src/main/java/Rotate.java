...
public class Rotate {
    ...
    static int[] findPositionsForIter(int[] arr, int x) {
        Pair[] pairs = new Pair[arr.length];
        for (int i = 0; i < arr.length; i++) {
            pairs[i] = new Pair(arr[i], i);
        }
        int[] idxArr = new int[x];
        int iter = 0;
        while (iter < x) {
            Pair max = new Pair(-1, -1);
            int effetiveArrlen = pairs.length - iter;
            Queue<Pair> q = new ArrayDeque<Pair>();
            int j = 0;
            for (int i = 0; i < effetiveArrlen; i++) {
                if (i < x) {
                    if (pairs[i].value > max.value) {
                        max = pairs[i];
                    }
                    q.add(pairs[i]);
                } else {
